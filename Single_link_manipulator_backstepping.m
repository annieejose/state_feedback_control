...
xlabel('time'); 
legend('X_1','X_2'); 
%Plot the control input - Current 
Current = U_ARR ; 
...
xlabel('time'); 
function [ x,t,u ] = eulr2(x0,P,t0,M,N,B) 
h=0.01; 
[k,u]=state_equation2(x0,P,M,N,B); 
x = x0  + k.*h; 
t = t0 + h; 
end
function [XDOT,U] = state_equation2(X,K,M,N,B) 
U = M*(X(1)+X(2))*(-1-K) + N*sin(X(1)) + B*X(2); 
X1DOT =  X(2); 
